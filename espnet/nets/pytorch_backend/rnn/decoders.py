...
class Decoder(torch.nn.Module, ScorerInterface):
    ...
    def __init__(
        ...
        if meetingKB is not None:
            ...
            self.char_emb_matrix = torch.nn.Embedding(meetingKB.vocab.ntokens, lm_odim)
            self.Qproj = torch.nn.Linear(self.dunits+eprojs, self.attn_dim)
            if self.wordemb > 0:
                self.CharembProj = torch.nn.Linear(lm_odim, self.wordemb)
    ...
    def get_meetingKB_emb(self, query, meeting_KB, meeting_mask, ptr_inds, factor=1.0, att_labs_seq=None):
        ...
        KBembedding = torch.einsum('ijk,ij->ik', meeting_KB, KBweight) # * factor
    ...
    def meeting_lextree_step(self, char_idx, new_tree, meeting):
        ...
        fulllist = list(range(self.meetingKB.maxlen))
        max_KBsize = self.meetingKB.maxlen
        unkidx = self.meetingKB.unkidx
        ptr_gen = True
        if char_idx in [self.eos, self.spaceids] or self.char_list[char_idx].endswith('▁'):
            new_tree = self.meetingKB.meetinglextree[meeting].copy()
            step_inds = deepcopy(fulllist[:max_KBsize-1]+[unkidx])
            step_mask = deepcopy(self.meetingKB.meetingmask[meeting])
            ptr_gen = True
        elif char_idx not in new_tree:
            new_tree = [{}]
            step_inds = [unkidx] * max_KBsize
            step_mask = [0] + [1] * (max_KBsize - 1)
            ptr_gen = False
        else:
            new_tree = new_tree[char_idx]
            start, end = new_tree[2]
            step_inds = fulllist[start:end] + [unkidx] * (max_KBsize - end + start)
            step_mask = [0] * (end - start + 1) + [1] * (max_KBsize - end + start - 1)
        return new_tree, torch.LongTensor([step_inds]), torch.LongTensor([step_mask]).byte(), ptr_gen

    def get_next_char_matrix(self, new_tree, max_KBsize, chardist=None):
        if chardist is None:
            char_mask_matrix = torch.zeros(max_KBsize, len(self.char_list))
            to_device(self, char_mask_matrix)
        else:
            char_mask_matrix = chardist
        cursor = 0
        if len(new_tree) > 1 and new_tree[1] != -1 and self.spaceids != -1:
            char_mask_matrix[0, self.spaceids] = 1.0
            cursor += 1
        for i, char in enumerate(self.char_list):
            if i in new_tree[0]:
                duration = new_tree[0][i][2][1] - new_tree[0][i][2][0]
                char_mask_matrix[cursor:cursor+duration, i] = 1.0
                cursor += duration
        return char_mask_matrix

    def get_all_meeting_lextree_embs(self, ylist, meetings, maxlen):
        batch_seq_inds = []
        batch_masks = []
        p_gen_mask = []
        ...
        fulllist = list(range(self.meetingKB.maxlen))
        max_KBsize = self.meetingKB.maxlen
        char_dist_list = torch.zeros([len(ylist), maxlen, max_KBsize, len(self.char_list)],
        ...
        for i, yseq in enumerate(ylist):
            ...
            meeting = meetings[i]
            lextree = self.meetingKB.meetinglextree[meeting]
            ...
            for j, char_idx in enumerate(yseq):
                ...
                if char_idx in [self.eos, self.spaceids] or self.char_list[char_idx].endswith('▁'):
                    new_tree = lextree.copy()
                    seq_of_inds.append(fulllist[:max_KBsize-1]+[unkidx])
                    masks_list.append(deepcopy(self.meetingKB.meetingmask[meeting]))
                    p_gen.append(0)
                elif char_idx not in new_tree:
                    new_tree = [{}]
                    seq_of_inds.append([unkidx] * max_KBsize)
                    masks_list.append([0] + [1] * (max_KBsize - 1))
                    p_gen.append(1)
                else:
                    new_tree = new_tree[char_idx]
                    start, end = new_tree[2]
                    seq_of_inds.append(fulllist[start:end] + [unkidx] * (max_KBsize - end + start))
                    masks_list.append([0] * (end - start + 1) + [1] * (max_KBsize - end + start - 1))
                    p_gen.append(0)
                # for pointer generator
                if self.PtrGen:
                    self.get_next_char_matrix(new_tree, max_KBsize, char_dist_list[i, j])
            pad_len = max_KBsize
            seq_of_inds += [[self.meetingKB.unkidx] * pad_len] * (maxlen - len(seq_of_inds))
            masks_list += [[1] * pad_len] * (maxlen - len(masks_list))
            batch_seq_inds.append(seq_of_inds)
            batch_masks.append(masks_list)
            # for pointer generator
            if self.PtrGen:
                p_gen_mask.append(p_gen + [1] * (maxlen - len(p_gen)))
        batch_seq_inds = to_device(self, torch.LongTensor(batch_seq_inds))
        batch_masks = to_device(self, torch.Tensor(batch_masks)).byte()
        if self.PtrGen:
            # char_dist_list = to_device(self, char_dist_list)
            p_gen_mask = to_device(self, torch.Tensor(p_gen_mask)).byte()

        return batch_seq_inds, batch_masks, char_dist_list, p_gen_mask

    def calc_ptr_loss(self, ptr_dist_all, ptr_inds, model_dist, ptr_gen, ptr_gen_mask,
        ...
        ptr_gen = torch.cat(ptr_gen, dim=1).masked_fill_(to_device(self, ptr_gen_mask), 0).view(-1, 1)
        # (bsz * seqlen * KBsize), (bsz * seqlen * KBsize * odim) -> (bsz * seqlen * odim)
        ptr_dist = torch.einsum('ijk,ijkl->ijl', ptr_dist, ptr_inds).view(targets.size(0), -1)
        # the gap to 1 is the prob for <unk>, which indicates not in the KB
        ptr_gen_complement = (1 - ptr_dist.sum(1, keepdim=True)) * ptr_gen
        model_dist = F.softmax(model_dist, dim=-1)
        p_final = ptr_dist * ptr_gen + model_dist * (1 - ptr_gen + ptr_gen_complement)
    ...
    def forward(self, hs_pad, hlens, ys_pad, strm_idx=0, lang_ids=None, meeting_info=None,
        ...
        if meeting_info is not None and self.epoch >= self.PtrSche:
            meeting_KB = self.char_emb_matrix.weight[meeting_info[0]]
            # Do projection only when using dot-product attention
            if self.KBlextree:
                lex_embs, lex_masks, ptr_inds, ptr_mask = self.get_all_meeting_lextree_embs(
                    ys_in, meeting_info[2], ys_in_pad.size(-1))
                if att_labs is not None:
                    att_lab_mask = att_labs == (self.meetingKB.maxlen - 1)
                    att_labs = (lex_embs == att_labs.unsqueeze(-1)).masked_fill_(lex_masks, 0)
                    att_labs.masked_fill_(att_lab_mask.unsqueeze(-1), 0)
                    att_labs = att_labs.float()
        ...
        for i in six.moves.range(olength):
            ...
            if meeting_info is not None and self.meetingKB is not None and self.epoch >= self.PtrSche:
                factor = 0.0 if (self.PtrSche > 0 and self.epoch < self.PtrSche) else 1.0
                ...
                if self.PtrGen:
                    p_gen = torch.sigmoid(self.pointer_gate(torch.cat((z_list[-1] if self.ac_only else query,
                        KBembedding), dim=1)))
                    # p_gen = torch.sigmoid(self.pointer_gate(torch.cat((query, KBembedding), dim=1)))
        ...
        if meeting_info is not None and self.PtrGen and self.epoch >= self.PtrSche:
            self.loss, loss_sep, KB_loss, pfinal = self.calc_ptr_loss(ptr_dist_all, ptr_inds, y_all, p_gen_all, ptr_mask,
                ys_out_pad.view(-1), self.ignore_id, reduction_str, separate=False, att_labs=att_labs)
        else:
    ...
    def recognize_beam(self, h, lpz, recog_args, char_list, rnnlm=None, strm_idx=0, meeting_info=None):
        ...
        if rnnlm:
            hyp = {
                ...
                "a_prev": a,
                "rnnlm_prev": None,
                "a_accum": torch.zeros(h[0].size(0), dtype=torch.float32),
                "final_score": 0.0,
            }
        else:
            hyp = {
        ...
        for i in six.moves.range(maxlen):
            ...
            for hyp in hyps:
                ...
                if self.meetingKB is not None and meeting_info != []:
                    if self.KBlextree:
                        tree_track, lex_inds, lex_mask, ptr_mask = self.meeting_lextree_step(
