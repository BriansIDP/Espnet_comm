...
lmtag=char_lm              # tag for managing LMs
use_lm=true
...
# exp tag
tag="atten_only" # tag for managing experiments.
...
# AMI_DIR=/home/dawna/nst/yq236/tools/kaldi-trunk-git/egs/ami/s5/data/ihm/train/ch1wav
# AMI_DIR_DEV=/home/dawna/nst/yq236/tools/kaldi-trunk-git/egs/ami/s5/data/ihm/dev/ch1wav
# AMI_DIR_EVAL=/home/dawna/nst/yq236/tools/kaldi-trunk-git/egs/ami/s5/data/ihm/eval/ch1wav
AMI_DIR=/scratch/ql264/data/AMI/
AMI_DIR_DEV=/scratch/ql264/data/AMI/
AMI_DIR_EVAL=/scratch/ql264/data/AMI/
# case $(hostname -d) in
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    local/ami_${base_mic}_scoring_data_prep.sh ${AMI_DIR_DEV} ${mic} dev
    local/ami_${base_mic}_scoring_data_prep.sh ${AMI_DIR_EVAL} ${mic} eval
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for rtask in ${recog_set}; do
        ...
        data2json.sh --feat ${feat_recog_dir}/feats.scp --allow-one-column true \
            --bpecode ${bpemodel}.model data/${rtask} ${dict} > ${feat_recog_dir}/data_${bpemode}${nbpe}${suffix}.json
    done
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    (
        if [ ${use_lm} = true ]; then
            ...
            decode_dir=decode_${rtask}_no_lm
        fi
        echo "Loading LM from ${lmexpdir}"
        if [ ${use_lm} = true ]; then
            if [ ${use_wordlm} = true ]; then
        ...
        ngpu=0
        decode_cmd="queue.pl -l qp=low,osrel=* -P black-svr"
